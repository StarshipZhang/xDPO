...
model_AutoMode = AutoModel.from_pretrained(model_pretrained_name_or_path).eval().to(device)
...
for model_id, data in score.items():
    ...
    for style , res in data.items():
        ...
        print(model_id, '{:<15}'.format(style), '{:.2f}'.format(torch.mean(avg_score_stack)), '\t', '{:.4f}'.format(torch.mean(avg_score_stack)))
    print(model_id, '{:<15}'.format('Average'), '{:.2f}'.format(torch.mean(all_score_stack)), '\t')
